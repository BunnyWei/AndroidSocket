 ...
 */
public class sendThread7 implements Runnable {
    ...
    public void run(){
        ...
        try {
            ...
            while(packetCount < 150){
                ...
                if(!calcuCRC(CRCcheckStr)){
                    Log.i("fail CRC: the number:",sendStr);
//                    byte[] ZeroByte = new byte[162];
//                    tempByte[packetCount] = ZeroByte;
                    if(packetCount >= 1){
                        tempByte[packetCount] = tempByte[packetCount -1];
                    }else tempByte[packetCount] = inputWav;
